...
pub mod wifi;
// LOCAL PATCH (RCD): the upstream gate hard-excluded esp32h2 and required Wi-Fi,
// so the high-level Thread stack (EspThreadMatterStack/EspMatterThread, in
// wireless/thread.rs) was unavailable on the pure-Thread ESP32-H2. The module's
// top-level code is transport-agnostic and its wifi/thread submodules are
// independently gated, so we relax this to "Wi-Fi OR (OpenThread + 802.15.4)".
// See SETUP.md "Vendored esp-idf-matter patch". Revert when upstream supports H2 Thread.
#[cfg(all(
    not(esp32s2),
    esp_idf_comp_esp_event_enabled,
    not(esp_idf_btdm_ctrl_mode_br_edr_only),
    ...
    any(
        esp_idf_comp_esp_wifi_enabled,
        all(esp_idf_comp_openthread_enabled, esp_idf_soc_ieee802154_supported),
    ),
))]
pub mod wireless;
