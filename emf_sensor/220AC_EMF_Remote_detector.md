...
* Capacitors:
  * Two 0.1 uF (104) ceramic (input DC-block + Pin-3 power-noise filter)
  ...
  Pin 3 (+) ─────┴─────[10kΩ]───── GND
               ...
               [0.1uF]
                ...
                GND
                 ...
                 │        ┌────[2.2nF]────┐          │
                 │        │               │          │
  Antenna ─[0.1uF]── Pin 2 (-) ───────────┴──────────┤─ Pin 1 (Out) ─[100Ω]─> Signal wire
                 │                                    │
                [100kΩ]                               │
                 │                                    │
                 └────────────────────────────────────┘
...
(Pin 1 <-> Pin 2). With the input high-pass (0.1 uF + 100 kΩ ~ 16 Hz) it makes the stage a
...
* **Pin 3 (Non-Inverting Input):** middle of the two-10 kΩ divider (one to 3.3 V, one to
  GND -> ~1.65 V). 0.1 uF from Pin 3 to GND filters power noise.

### 2. Input & Feedback Loop
* **Pin 2 (Inverting Input):** the copper antenna through a 0.1 uF DC-blocking cap; a
...
* **See the live readings:** build with debug logging - `./build.sh -d run
