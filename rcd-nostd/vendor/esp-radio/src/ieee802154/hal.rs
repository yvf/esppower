...
///
/// Mirrors the C driver's `ieee802154_ll_enhack_generate_done_notify()`, which
/// writes `IEEE802154.enhack_generate_done_notify = true`. In this PAC that
/// register is `enhance_ack_cfg` with the single field
/// `tx_enh_ack_generate_done_notify`; writing 1 strobes it.
#[inline(always)]
